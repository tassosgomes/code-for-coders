...
|---|---|---|---|---|
| [api-contract.yaml](api-contract.yaml) e [api-contract.md](api-contract.md) | OpenAPI 3.1.0 | 1.0.1 | Recorte de CAP-002 na borda `admin-spa` → `bff-admin` | **Aprovado para implementação** em 2026-09-25; lint sem erros |
...
| [internal-api-contract.yaml](internal-api-contract.yaml) e [internal-api-contract.md](internal-api-contract.md) | OpenAPI 3.1.0 | 1.0.1 | Recorte `bff-admin` → Identity: 13 operações de backoffice, validação de sessão com papéis/permissões e JWT, JWKS | **Aprovado para implementação** em 2026-09-25; lint sem erros |
...
Decisão de evolução, aprovada pelo usuário em 2026-09-25 (revisão da task 7.0, bloqueante B1):

| ID | Decisão | Alternativa descartada |
|---|---|---|
| C-11 | **Troca com `fromRole` igual a `toRole` responde 422 `ROLE_CHANGE_INVALID`** em `changeStaffRole` e `changeStaffRoleInternal`. O 422 já estava no contrato público 1.0.0 sem `code`; 1.0.1 fixa o código. Na borda pública o código é repassado sem tradução | Reusar `ROLE_NOT_HELD`: a conta tem o papel, a regra violada é outra. Tratar como sem efeito (200, `changed: false`): o pedido é inválido, não um estado já atingido |
...
- **1.0.0 → 1.0.1 (público e interno de Identity, C-11):** aditiva. O status 422 para `fromRole` igual a `toRole` já constava do público; 1.0.1 acrescenta o `code` `ROLE_CHANGE_INVALID` à lista do `BusinessRule` interno e um exemplo no público. `Problem.code` é string livre, então o schema não muda; consumidores (`admin-spa`, `bff-admin`) devem tratar o código como os demais 422 da troca. A 1.0.0 não foi integrada à `main` antes desta revisão.
...
|---|---|---|---|
| [api-contract.yaml](api-contract.yaml) | `npx --yes @stoplight/spectral-cli@6.15.0 lint tasks/prd-acesso-interno/api-contract.yaml --ruleset .claude/skills/tsg-flow-contract-creator/rulesets/openapi.yaml --fail-severity=error` | OpenAPI 3.1.0, ruleset local | Sem erros nem avisos (um erro de exemplo corrigido na primeira execução); 1.0.1 revalidada em 2026-09-25 sem erros |
| [internal-api-contract.yaml](internal-api-contract.yaml) | `npx --yes @stoplight/spectral-cli@6.15.0 lint tasks/prd-acesso-interno/internal-api-contract.yaml --ruleset .claude/skills/tsg-flow-contract-creator/rulesets/openapi.yaml --fail-severity=error` | OpenAPI 3.1.0, ruleset local | Sem erros nem avisos; 1.0.1 revalidada em 2026-09-25 sem erros |
