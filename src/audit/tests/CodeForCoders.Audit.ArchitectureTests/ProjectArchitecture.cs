...
namespace CodeForCoders.Audit.ArchitectureTests;
...
internal static class ProjectArchitecture
{
    ...
    private static readonly Assembly InfraDataAssembly = typeof(AuditDbContext).Assembly;
    private static readonly Assembly InfraMessagingAssembly = typeof(AuditEventConsumerWorker).Assembly;
    private static readonly Assembly ApiAssembly = typeof(AuditSmokeEndpoints).Assembly;
    ...
    public static readonly IObjectProvider<IType> UseCases =
        Types().That().HaveFullNameContaining("CodeForCoders.Audit.Application.UseCases.").As("Use cases");
    public static readonly IObjectProvider<IType> ApiExtensions =
        Types().That().HaveFullNameContaining("CodeForCoders.Audit.Api.Extensions.").As("Composition root");
    public static readonly IObjectProvider<IType> Endpoints =
        Types().That().HaveFullNameContaining("CodeForCoders.Audit.Api.Endpoints.").As("Endpoints");
    public static readonly IObjectProvider<IType> ApiContracts =
        Types().That().HaveFullNameContaining("CodeForCoders.Audit.Api.ApiModels.").As("HTTP contracts");
    public static readonly IObjectProvider<IType> DomainEntities =
        Types().That().HaveFullNameContaining("CodeForCoders.Audit.Domain.Entities.").As("Domain entities");
    public static readonly IObjectProvider<IType> PersistencePorts =
        Types().That().HaveFullNameContaining("CodeForCoders.Audit.Domain.Repositories.")
            .Or().HaveFullNameContaining("CodeForCoders.Audit.Application.Interfaces.IUnitOfWork")
            .Or().HaveFullNameContaining("CodeForCoders.Audit.Application.Interfaces.IAuditRecordWriter")
