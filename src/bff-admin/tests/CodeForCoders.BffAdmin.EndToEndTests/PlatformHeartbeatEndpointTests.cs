...
public sealed class PlatformHeartbeatEndpointTests(BffAdminApiFactory factory)
{
    [Fact]
    public async Task PostHeartbeat_ReturnsAcceptedAndPersistsOutboxMessage()
    {
        ...
        Assert.Equal("bff-admin.platform.heartbeat.v1", responseBody.RoutingKey);
        ...
        Assert.Equal("bff-admin.platform.heartbeat.v1", message.RoutingKey);
    }
