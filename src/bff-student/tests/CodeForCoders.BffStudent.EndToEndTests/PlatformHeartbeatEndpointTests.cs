...
public sealed class PlatformHeartbeatEndpointTests(BffStudentApiFactory factory)
{
    [Fact]
    public async Task PostHeartbeat_ReturnsAcceptedAndPersistsOutboxMessage()
    {
        ...
        Assert.Equal("bff-student.platform.heartbeat.v1", responseBody.RoutingKey);
        ...
        Assert.Equal("bff-student.platform.heartbeat.v1", message.RoutingKey);
    }
