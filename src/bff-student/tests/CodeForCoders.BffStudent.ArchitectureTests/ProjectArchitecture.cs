...
namespace CodeForCoders.BffStudent.ArchitectureTests;
...
internal static class ProjectArchitecture
{
    ...
    private static readonly Assembly InfraDataAssembly = typeof(BffStudentDbContext).Assembly;
    ...
    public static readonly IObjectProvider<IType> UseCases =
        Types().That().HaveFullNameContaining("CodeForCoders.BffStudent.Application.UseCases.").As("Use cases");
    public static readonly IObjectProvider<IType> ApiExtensions =
        Types().That().HaveFullNameContaining("CodeForCoders.BffStudent.Api.Extensions.").As("Composition root");
    public static readonly IObjectProvider<IType> Endpoints =
        Types().That().HaveFullNameContaining("CodeForCoders.BffStudent.Api.Endpoints.").As("Endpoints");
    public static readonly IObjectProvider<IType> ApiContracts =
        Types().That().HaveFullNameContaining("CodeForCoders.BffStudent.Api.ApiModels.").As("HTTP contracts");
    public static readonly IObjectProvider<IType> DomainEntities =
        Types().That().HaveFullNameContaining("CodeForCoders.BffStudent.Domain.Entities.").As("Domain entities");
    public static readonly IObjectProvider<IType> PersistencePorts =
        Types().That().HaveFullNameContaining("CodeForCoders.BffStudent.Domain.Repositories.")
            .Or().HaveFullNameContaining("CodeForCoders.BffStudent.Application.Interfaces.IUnitOfWork")
            .Or().HaveFullNameContaining("CodeForCoders.BffStudent.Application.Interfaces.IOutboxMessageWriter")
