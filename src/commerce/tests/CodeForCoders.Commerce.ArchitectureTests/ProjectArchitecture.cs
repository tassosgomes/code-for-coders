...
internal static class ProjectArchitecture
{
    ...
    private static readonly Assembly InfraDataAssembly = typeof(CommerceDbContext).Assembly;
    ...
    public static readonly IObjectProvider<IType> UseCases =
        Types().That().HaveFullNameContaining("CodeForCoders.Commerce.Application.UseCases.").As("Use cases");
    public static readonly IObjectProvider<IType> ApiExtensions =
        Types().That().HaveFullNameContaining("CodeForCoders.Commerce.Api.Extensions.").As("Composition root");
    public static readonly IObjectProvider<IType> Endpoints =
        Types().That().HaveFullNameContaining("CodeForCoders.Commerce.Api.Endpoints.").As("Endpoints");
    public static readonly IObjectProvider<IType> ApiContracts =
        Types().That().HaveFullNameContaining("CodeForCoders.Commerce.Api.ApiModels.").As("HTTP contracts");
    public static readonly IObjectProvider<IType> DomainEntities =
        Types().That().HaveFullNameContaining("CodeForCoders.Commerce.Domain.Entities.").As("Domain entities");
    public static readonly IObjectProvider<IType> PersistencePorts =
        Types().That().HaveFullNameContaining("CodeForCoders.Commerce.Domain.Repositories.")
            .Or().HaveFullNameContaining("CodeForCoders.Commerce.Application.Interfaces.IUnitOfWork")
            .Or().HaveFullNameContaining("CodeForCoders.Commerce.Application.Interfaces.IOutboxMessageWriter")
