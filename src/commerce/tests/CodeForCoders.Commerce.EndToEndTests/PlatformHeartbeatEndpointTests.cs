...
public sealed class PlatformHeartbeatEndpointTests(CommerceApiFactory factory)
{
    [Fact]
    public async Task PostHeartbeat_ReturnsAcceptedAndPersistsOutboxMessage()
    {
        ...
        Assert.Equal("commerce.platform.heartbeat.v1", responseBody.RoutingKey);
        ...
        Assert.Equal("commerce.platform.heartbeat.v1", message.RoutingKey);
    }
