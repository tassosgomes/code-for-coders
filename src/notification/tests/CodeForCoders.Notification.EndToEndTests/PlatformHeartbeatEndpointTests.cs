...
namespace CodeForCoders.Notification.EndToEndTests;

[Collection(NotificationApiCollection.Name)]
public sealed class PlatformHeartbeatEndpointTests(NotificationApiFactory factory)
{
    [Fact]
    public async Task PostHeartbeat_ReturnsAcceptedAndPersistsOutboxMessage()
    {
        ...
        Assert.Equal("notification.platform.heartbeat.v1", responseBody.RoutingKey);
        ...
        Assert.Equal("notification.platform.heartbeat.v1", message.RoutingKey);
    }
