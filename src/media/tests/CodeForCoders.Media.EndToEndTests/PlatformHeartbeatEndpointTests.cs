...
public sealed class PlatformHeartbeatEndpointTests(MediaApiFactory factory)
{
    [Fact]
    public async Task PostHeartbeat_ReturnsAcceptedAndPersistsOutboxMessage()
    {
        ...
        Assert.Equal("media.platform.heartbeat.v1", responseBody.RoutingKey);
        ...
        Assert.Equal("media.platform.heartbeat.v1", message.RoutingKey);
    }
